...
const SignInPage = () => {
  ...
  const handleSubmit = async (data: SignInDataT) => {
    ...
    if (result?.data?.token?.length) {
      ...
      dispatch(
        setCredentials({ user: { email }, accessToken: result?.data?.token })
      );
      navigateTo("/");
    } else {
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center text-center py-16 px-6">
      <div className="w-full max-w-[540px]">
        <h1 className="text-[26px] text-[#323B4B] font-bold">Sign In</h1>
